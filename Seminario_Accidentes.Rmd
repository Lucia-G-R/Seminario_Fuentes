...
### **Summariese**
Calculamos la media de accidentes laborales en los Años 2004 y 2009 con el dataframe conjunto del conjunto de datos de salud mental y el número de accidentes labotrales.
```{r, warning=FALSE}
mediana_acci<- 
  df_Salu_Arag%>%
  group_by(Años)%>%
  summarise(Mediana_Accidentes = median(NumAcci, na.rm = TRUE))

datatable(mediana_acci)
```
...
# Filtramos las filas correspondientes a problemas óseos y agrupamos por sexo y condición
df_resultado <- df_cana_Esp %>%
  filter(Nombre == "Problema óseo, articular o muscular que afecta principalmente a las caderas, las piernas o los pies" | 
         Nombre == "Problema óseo, articular o muscular que afecta principalmente al cuello, los hombros, los brazos o l") %>%
  group_by(Sexo, Nombre) %>%
  summarise(casos = n(), .groups = 'drop')

# Crear el gráfico
grafico <- ggplot(df_resultado, aes(x = Sexo, y = casos, fill = Nombre)) + 
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = 'Sexo', 
       y = 'Número de casos', 
       title = 'Número de casos de problemas óseos según el sexo',
       fill = 'Condición') + 
  theme_bw() +
