...
Salud <- rename(.data = Salud_df, TipoEnf = `T3_Variable`, Indicador = `Codigo`, Variable = `..JSON_jsonT3_Variable`, Nombre = `..JSON_jsonNombre`, Codigo = `..JSON_jsonCodigo`, jsonValor = `..JSON_jsonValor`)
...
Eliminamos los valores nulos del conjunto de datos sobre los accidentes laborales que suceden en Canarias.
```{r}
(canarias, na.rm = TRUE)
```
...
espana <- 
  espana_df%>%
  select(TipoEnf, Indicador, Sexo, Obs)

Salud <-
  Salud_df%>%
  select(Sexo, Indicador, EstadoAnsiedad, Obs)
```
