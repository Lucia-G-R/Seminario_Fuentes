...
- https://datos.gob.es/es/catalogo/ea0010587-personas-de-16-a-74-ocupadas-anos-o-que-trabajaron-anteriormente-que-hayan-tenido-alguna-enfermedadlaboral-por-tipo-de-enfermedad-y-sexo-epa-identificador-api-t22-e308-meto_05-modulo-base_2011-2020-l0-02003-px1
- https://datos.gob.es/es/catalogo/a05003423-poblacion-de-16-y-mas-anos-segun-autovaloracion-del-estado-de-salud-en-relacion-a-situaciones-de-ansiedad-o-depresion-sexos-o-grupos-de-edad-por-anos-canarias-2004-2009
- https://datos.gob.es/es/catalogo/a05003423-accidentes-de-trabajo-en-jornada-segun-tipos-de-trabajo-gravedad-trabajo-habitual-o-no-y-sexos-canarias-y-anos
- https://datos.gob.es/es/catalogo/a02002834-accidentes-en-jornada-de-trabajo-n-de-accidentes-segun-grado-de-lesion-y-sexo
...
canarias <- 
  fromJSON(file = "INPUT/DATA/Accidentes_laborales_Canarias.json")

canarias

# Como las sublistas tienen diferentes longitudes las relleno para poder realizar un data.frame
max_length<-max(sapply(canarias, length)) # Encontrar la longitud maxima

canarias_mod<-lapply(canarias, function(x){
  length(x)<-max_length
  return(x) # Sublista ajustada
})

canarias_mod<-canarias_mod[sapply(canarias_mod, function(x) length(x)> 0)]

# Convertir la lista de listas en data.frame
canarias_df<-do.call(rbind, lapply(canarias_mod, as.data.frame))
...
```{r}
canarias_df<-as.data.frame(canarias)

Aragon_df<-as.data.frame(Aragon)

España_df<-as.data.frame(España)
