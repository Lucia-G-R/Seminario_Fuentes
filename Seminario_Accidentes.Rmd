...
datatable(Salud_df)
```
Este dataframe va a recoger los distintos estados de ánimo durante el año 2004 y 2009 de los ciudadnos de Canarias con más de 16 años, en ambos sexos.
...
```
Con este dataframe pretendemos que de forma visual podamos observar como puede llegar a afectar el padecimiento de una enfermedad mental en el desarrollo de enfermedades laborales por cada profesión estudiada.
...
En primer lugar, vamos a eliminar en el dataframe Salud, el sexo que indica las edades ya que es una información no correcta para esta variable. Utilizamos la función subset.
```{r, warning=FALSE, error = TRUE}
library(DT)

# Normalizamos los valores y eliminamos los espacios
Salud$Sexo <- trimws(Salud$Sexo)
Salud$Sexo <- tolower(Salud$Sexo)

Salud_eli <- subset(Salud, !Sexo %in% c("De 16 a 64 a\xf1os", "De 65 y m\xe1s a\xf1os"))
...
print (grafico_f1)
```
Como resultado, podemos observar que en hombres hay más accidentes laborales que en el sexo contrario y en ambos casos estos son de gravedad leve.

### **Gráfico sobre los nieveles de ansiedad en mujeres**
Este gráfico muestra la diferencia de casos de ansiedad en mujeres y el grado de ansiedad.
Hemos querido eliminar los indiviudos que no saben (o no contesta) junto con el total ya que no daban información de valor. Nos hemos quedado con las muejres ya que realmente proporcionan alguna conclusión. 
```{r}
...
print(grafico_f2)
```
Como resultado, podemos observar que los años donde las mujeres han presentado menor ansiedad han sido en 2009, 2017, 2018 y 2019. Sin embargo el resto de los años estudiados presentan un incremento en el dearrollo de esta enfermedad.
...
Las resùestas estan respondidas abajo

- ¿Existe relación entre la profesión de movimiento de tierra respecto al género? *Grafico 1
- ¿Ha aumentado el número de accidentes laborales? *Grafico 2
- ¿Existe relación entre la profesión de movimiento de tierra respecto a la indicencia de accidentes? *Grafico 3

# **Conclusiones**

Como se ha observado en los gráficos presentados anteriormente, el género masculino muestra mayor indicencia de de accidentes en la profesión relacionada con el movimiento de tierra. Este fenómeno se puede explicar a que este grupo representa la mayoria en esta actividad laboral (articulo1)
...
Por otro lado, cabe señalar que esta profesión no se encuentra entre las mas afectadas por los siniestros laborales. La mayoria de incidentes se han registrado en el sector vinculado con la "producción, transformación, tratamiento y almacenamiento de materiales". 
Destacar que estos útimos años, se ha presentado un aumento en las bajas laborales en el ámbitorelacionado con los "servicios a empresas y trabajos intelectuales"

En conclusión, la profesión objeto de este estudio ha experimentado un elevado numero de daños en el ámbito laboral a principios del siglo XXI.
Sin embargo, las cifras han ido disminuyendo progresivamente y manteniendose estables en la actualidad.
...
*Articulo trabajadores construcción (1)*
