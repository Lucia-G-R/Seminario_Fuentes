...
```
#### **Slice_sample**
Escogemos un conjunto de filas, exactamente 10 filas aleatorias del conjunto de datos de Espana
```{r, warning=FALSE}
library("DT")

datatable(slice_sample(.data = Espana, n = 10))
```
#### **Slice_max y slice_min**
```{r, warning=FALSE}
library("DT")

datatable(slice_max(.data = canarias))
```

```{r, warning=FALSE}
library("DT")

datatable(slice_min(.data = canarias))
```
...
df_cana_Esp <- 
  canarias_dis %>%
  inner_join(Espana_dis, by = c("Año" = "Años"))
