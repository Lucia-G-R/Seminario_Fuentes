...
# Extraemos la variable "Nombre" del JSON que es un caracter
nombre <- espana_json$Nombre

# HASTA AQUÍ DA BIEN
# Unimos MetaData_df y Data_df usando "T3_Variable" como clave
espana_df <- 
  MetaData_df %>%
  left_join(Data_df, by = "T3_Variable")

# Añadimos una columna con el nombre del dataset
espana_df <- espana_df %>%
  mutate(Nombre_Dataset = nombre)

espana_df<-
  espana_json%>%
  select(document.id)%>%
  left_join(MetaData_df, by = "..JSON")%>%
  left_join(Data_df, by = "..JSON")
